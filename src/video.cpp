...
ros::Publisher qrAdjust_pub;
ros::Publisher qrSpotted_pub;
//std::vector prevQr;
...
int main ( int argc, char **argv )
{
	...
	ros::Subscriber frontImageRaw_sub = nh.subscribe("ardrone/bottom/image_raw", 1000, imageCallback);
	 */
	ros::spin();
...
void imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
	//http://dsynflo.blogspot.dk/2014/10/opencv-qr-code-detection-and-extraction.html
	...
	try
	{
		...
		Mat gray(cv_ptr->image.size(), CV_MAKETYPE(cv_ptr->image.depth(), 1));			// To hold Grayscale Image
		Mat edges(cv_ptr->image.size(), CV_MAKETYPE(cv_ptr->image.depth(), 1));			// To hold Grayscale Image
		Mat traces(cv_ptr->image.size(), CV_8UC3);								// For Debug Visuals
		Mat qr,qr_raw,qr_gray,qr_thres,qr_balanced;
		...
		int DBG=1;						// Debug Flag
			...
			qr_raw = Mat::zeros(100, 100, CV_8UC3 );
			qr_balanced = Mat::zeros(100, 100, CV_8UC3 );

		//cv::xphoto::balanceWhite(framePtr->image, outPtr, cv::xphoto::WHITE_BALANCE_SIMPLE);

			//capture >> image;						// Capture Image from Image Input

			cvtColor(cv_ptr->image,gray,CV_RGB2GRAY);		// Convert Image captured from Image Input to GrayScale
			...
			for( int i = 0; i < contours.size(); i++ )
			{
				...
				// Tjek om der er nested childs i det lukkede område.
				while(hierarchy[k][2] != -1)
				{
					// Vi definerer k til at være denne ydre ring. Kigger herefter på det lukkede område inde i.
					k = hierarchy[k][2] ;
					//Tæller en op på contours i hierakiet.
					c = c+1;
				}
				//
				if(hierarchy[k][2] != -1)
				...
				{
					//std::vector approx;
					//double peri = cv::arcLength(contour[i], true);
					//approx = cv2.approxPolyDP(contour[i],approx, 0.04 * peri, true);
					cv::Rect rect = cv::boundingRect(contours[i]);
					if (rect.width >= 0.8*rect.height && rect.width <= 1.2*rect.height) {


						if (mark == 0) {
							A = i;
							//areaA = cv::contourArea(contours[A]);
						}
						else if (mark == 1) {
							B = i;        // i.e., A is already found, assign current contour to B
							//areaB = cv::contourArea(contours[B]);
						}
						else if (mark == 2) {
							C = i;        // i.e., A and B are already found, assign current contour to C
							//areaC = cv::contourArea(contours[C]);
						}
						/*
                        else if (mark == 3){
                            //cout << "ja";
                            wrongMark = i;
                            double wrongMarkArea = cv::contourArea(contours[wrongMark]);

                            double upperA,upperB,upperC,lowerA,lowerB,lowerC;
                            upperA = areaA + (areaA*0.4);
                            lowerA = areaA - (areaA*0.4);
                            upperB = areaB + (areaB*0.4);
                            lowerB = areaB - (areaB*0.4);
                            upperC = areaC + (areaC*0.4);
                            lowerC = areaC - (areaC*0.4);

                            //cout << "Først: " << "A: " << areaA << "B: " << areaB << "C: " << areaC << "D: " << wrongMarkArea << endl;

                            if((wrongMarkArea >= lowerA && wrongMarkArea <= upperA) && (wrongMarkArea >= lowerB && wrongMarkArea <= upperB)){
                                C = wrongMark;
                            }
                            else if((wrongMarkArea >= lowerA && wrongMarkArea <= upperA) && (wrongMarkArea >= lowerC && wrongMarkArea <= upperC)){
                                B= wrongMark;
                            }
                            else if((wrongMarkArea >= lowerB && wrongMarkArea <= upperB) && (wrongMarkArea >= lowerC && wrongMarkArea <= upperC)){
                                A = wrongMark;
                            }
                            ...
                            //cout << "Efter: " << "A: " << areaA << "B: " << areaB << "C: " << areaC << "D: " << wrongMarkArea << endl;
			...
			if (mark >= 2)		// Ensure we have (atleast 3; namely A,B,C) 'Alignment Markers' discovered
			{
				...
				if (top < contours.size() && right < contours.size() && bottom < contours.size() &&
					...
					if (src.size() == 4 && dst.size() ==
										   4)            // Failsafe for WarpMatrix Calculation to have only 4 Points with src and dst
					{
						warp_matrix = getPerspectiveTransform(src, dst);
						warpPerspective(cv_ptr->image, qr_raw, warp_matrix, Size(qr.cols, qr.rows));
					...
					drawContours(cv_ptr->image, contours, top, Scalar(255, 200, 0), 2, 8, hierarchy, 0);
					drawContours(cv_ptr->image, contours, right, Scalar(0, 0, 255), 2, 8, hierarchy, 0);
					drawContours(cv_ptr->image, contours, bottom, Scalar(255, 0, 100), 2, 8, hierarchy, 0);
			...
			cv::xphoto::balanceWhite(qr_raw, qr_balanced, cv::xphoto::WHITE_BALANCE_SIMPLE);


		//QR code read
		Mat grey;
		cvtColor(qr_balanced,grey,CV_BGR2GRAY);
		int width = qr_balanced.cols;
		int height = qr_balanced.rows;
		uchar *raw = (uchar *)grey.data;
		// wrap image data
		Image imageQR(width, height, "Y800", raw, width * height);
		// scan the image for barcodes
		int n = scanner.scan(imageQR);

		// extract results
        string id;
		for(Image::SymbolIterator symbol = imageQR.symbol_begin();
            ...
            id=symbol->get_data();
			putText(cv_ptr->image, id, Point(20, 30), FONT_HERSHEY_PLAIN, 1, Scalar(0, 255, 0), 1, 8);
			}

        std::stringstream qq;
        qq << id;
        std_msgs::String spottedMsg;
        spottedMsg.data = qq.str();



        if(spottedMsg.data.compare("")){
            cout << spottedMsg.data << endl;
            pubLock.lock();
            qrSpotted_pub.publish(spottedMsg);
            pubLock.unlock();
        }
